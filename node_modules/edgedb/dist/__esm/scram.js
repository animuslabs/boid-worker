...
export function saslprep(str) {
    return str.normalize("NFKC");
}
export async function generateNonce(length = RAW_NONCE_LENGTH) {
    return await randomBytes(length);
}
export function buildClientFirstMessage(clientNonce, username) {
    const bare = `n=${saslprep(username)},r=${clientNonce.toString("base64")}`;
    return [`n,,${bare}`, bare];
}
export function parseServerFirstMessage(msg) {
...
export function parseServerFinalMessage(msg) {
    const attrs = msg.split(",");
    if (attrs.length < 1) {
        throw new ProtocolError("malformed SCRAM message");
    }
...
export function buildClientFinalMessage(password, salt, iterations, clientFirstBare, serverFirst, serverNonce) {
...
export function getSaltedPassword(password, salt, iterations) {
...
export function getClientKey(saltedPassword) {
    return HMAC(saltedPassword, Buffer.from("Client Key", "utf8"));
}
export function getServerKey(saltedPassword) {
