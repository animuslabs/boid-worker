...
import { allowsLiterals, generateFuncopDef, generateFuncopTypes, generateReturnCardinality, } from "./generateFunctionTypes";
...
import { OperatorKind } from "..";
import { getStringRepresentation } from "./generateObjectTypes";
export function generateOperatorFunctions({ dir, operators, types, casts, }) {
    generateFuncopTypes(dir, types, casts, operators, "Operator", "OpExpr", false, (code, opDef, args, namedArgs, returnType) => {
...
export function generateOperators({ dir, operators, types, casts, }) {
    ...
    for (const [opName, _opDefs] of operators.entries()) {
        if (skipOperators.has(opName))
            continue;
        const opDefs = expandFuncopAnytypeOverloads(sortFuncopOverloads(_opDefs, typeSpecificities), types, casts, implicitCastableRootTypes);
        let overloadIndex = 0;
        for (const opDef of opDefs) {
            ...
            if (opDef.overloadIndex === overloadIndex) {
                ...
                overloadDefs[opDef.operator_kind][opSymbol].push(generateFuncopDef(opDef));
                overloadIndex++;
            }
...
*/`,
            ...
            const returnAnytype = anytypes
                ? anytypes.kind === "castable"
                    ...
                    : `$.getPrimitive${anytypes.type[0] === "$.NonArrayType" ? "NonArray" : ""}BaseType<${allowsLiterals(anytypes.typeObj, anytypes)
                        ...
                        : anytypes.refName}${anytypes.refPath}>`
                : undefined;
            const returnType = getStringRepresentation(types.get(opDef.return_type.id), {
                types,
            ...
            overloadsBuf.indented(() => {
                ...
                overloadsBuf.writeln([
                    t `$.TypeSet<${returnType.staticType}, ${generateReturnCardinality(opName, params, opDef.return_typemod, false, anytypes)}>`,
                ]);
            });
            overloadsBuf.writeln([t `>;`]);
        }
