...
import { getRef, joinFrags, literalToScalarMapping, quote, scalarToLiteralMapping, } from "../util/genutil";
import { util } from "../util/util";
import { getStringRepresentation } from "./generateObjectTypes";
const getRuntimeRef = (name) => getRef(name, { prefix: "" });
export const generateCastMaps = (params) => {
    ...
    for (const outer of materialScalars) {
        assignableMap.writeln([
            t `  T extends ${getRef(outer.name)} ? ${getStringRepresentation(types.get(outer.id), {
                types,
    ...
    f.writeln([
        ...
        t `: B`,
        `)`,
        t `: ${joinFrags(["A", "B", ...[...returnTypes].map(type => getRef(type))], " | ")}`,
        r ` {`,
    ]);
  ...
  return _b != null ? _b : false;
    ...
    f.writeln([
        t `  ? T extends ${joinFrags([...types.values()]
            .filter(type => {
...
>;`,
    ...
    f.writeln([r `  }`]);
    for (const [literalType, { literalKind, type }] of Object.entries(literalToScalarMapping)) {
