...
export function toIdent(name) {
    if (name.includes("::")) {
        throw new Error(`toIdent: invalid name ${name}`);
    }
    return name.replace(/([^a-zA-Z0-9_]+)/g, "_");
}
export const makePlainIdent = (name) => {
...
for (const [scalarType, { type, literalKind }] of Object.entries(scalarToLiteralMapping)) {
    if (literalKind) {
...
export function toTSScalarType(type, types, opts = {
    ...
    switch (type.kind) {
        case "scalar": {
            ...
            const literalType = (_b = (_a = scalarToLiteralMapping[type.name]) === null || _a === void 0 ? void 0 : _a.type) !== null && _b !== void 0 ? _b : "unknown";
            return [
...
export function toTSObjectType(type, types, currentMod, code, level = 0) {
...
export function getInternalName({ fqn, id }) {
    const { name } = splitName(fqn);
    return makeValidIdent({ id, name });
}
export function makeValidIdent({ id, name, skipKeywordCheck, }) {
...
export function getRef(name, opts) {
    var _a;
    return {
        type: "identRef",
        name,
...
export function frag(strings, ...exprs) {
    const frags = [];
...
export function joinFrags(frags, sep) {
    const joined = [];
