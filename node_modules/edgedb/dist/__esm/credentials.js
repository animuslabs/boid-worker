...
export function validateCredentials(data) {
    ...
    if (certData != null) {
        ...
        if (caData != null && certData !== caData) {
            throw new InterfaceError(`both 'tls_ca' and 'tls_cert_data' are defined, ` +
    ...
    if (tlsSecurity != null &&
        (typeof tlsSecurity !== "string" ||
            !validTlsSecurityValues.includes(tlsSecurity))) {
        throw new InterfaceError(`\`tls_security\` must be one of ${validTlsSecurityValues
            .map(val => `"${val}"`)
            .join(", ")}`);
    }
    if (verifyHostname &&
        ...
        throw new InterfaceError(`both 'tls_security' and 'tls_verify_hostname' are defined, ` +
            `and are not in agreement`);
    }
