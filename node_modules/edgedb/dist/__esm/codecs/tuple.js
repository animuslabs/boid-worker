...
export class TupleCodec extends Codec {
    ...
    encode(buf, object) {
        ...
        if (object.length !== codecsLen) {
            throw new InvalidArgumentError(`expected ${codecsLen} tuple item${codecsLen === 1 ? "" : "s"}, got ${object.length}`);
        }
        if (!codecsLen) {
    ...
    encodeArgs(args) {
        ...
        if (args.length !== codecsLen) {
            throw new InvalidArgumentError(`expected ${codecsLen} argument${codecsLen === 1 ? "" : "s"}, got ${args.length}`);
        }
    ...
    decode(buf) {
        ...
        if (els !== subCodecs.length) {
            throw new ProtocolError(`cannot decode Tuple: expected ` +
...
export class EmptyTupleCodec extends Codec {
    encode(buf, object) {
        if (!Array.isArray(object)) {
            throw new InvalidArgumentError("cannot encode empty Tuple: expected an array");
        }
        if (object.length) {
            throw new InvalidArgumentError(`cannot encode empty Tuple: expected 0 elements got ${object.length}`);
        }
        buf.writeInt32(4);
        buf.writeInt32(0);
    }
    decode(buf) {
        ...
        if (els !== 0) {
            throw new ProtocolError(`cannot decode empty Tuple: expected 0 elements, received ${els}`);
        }
        return [];
    }
    getSubcodecs() {
