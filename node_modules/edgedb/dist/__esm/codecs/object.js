...
import { InvalidArgumentError, MissingArgumentError, QueryArgumentError, UnknownArgumentError, ProtocolError, } from "../errors";
const EDGE_POINTER_IS_IMPLICIT = 1 << 0;
const EDGE_POINTER_IS_LINKPROP = 1 << 1;
export class ObjectCodec extends Codec {
    ...
    _encodePositionalArgs(args) {
        ...
        if (args.length !== codecsLen) {
            throw new QueryArgumentError(`expected ${codecsLen} argument${codecsLen === 1 ? "" : "s"}, got ${args.length}`);
        }
        ...
        for (let i = 0; i < codecsLen; i++) {
            ...
            if (arg == null) {
                ...
                if (card === ONE || card === AT_LEAST_ONE) {
                    throw new MissingArgumentError(`argument ${this.fields[i].name} is required, but received ${arg}`);
                }
    ...
    _encodeNamedArgs(args) {
        if (args == null) {
            throw new MissingArgumentError("One or more named arguments expected, received null");
        }
        ...
        if (keys.length > codecsLen) {
            ...
            throw new UnknownArgumentError(`Unused named argument${extraKeys.length === 1 ? "" : "s"}: "${extraKeys.join('", "')}"`);
        }
        const elemData = new WriteBuffer();
        for (let i = 0; i < codecsLen; i++) {
            ...
            if (val == null) {
                const card = this.cardinalities[i];
                if (card === ONE || card === AT_LEAST_ONE) {
                    throw new MissingArgumentError(`argument ${this.fields[i].name} is required, but received ${val}`);
                }
                elemData.writeInt32(-1);
            }
            else {
    ...
    decode(buf) {
        ...
        if (els !== codecs.length) {
            throw new ProtocolError(`cannot decode Object: expected ${codecs.length} elements, got ${els}`);
        }
