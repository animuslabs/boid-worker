...
export class CodecsRegistry {
    ...
    setCustomCodecs({ decimal_string, int64_bigint, datetime_localDatetime, json_string, } = {}) {
        ...
        if (json_string) {
            this.customScalarCodecs.set(JSON_TYPEID, new JSONStringCodec(JSON_TYPEID));
        }
        else {
    ...
    _buildCodec(frb, cl, protocolVersion) {
        ...
        if (res != null) {
            switch (t) {
                ...
                case CTYPE_ARRAY: {
                    ...
                    const els = frb.readUInt16();
                    if (els !== 1) {
                        throw new ProtocolError("cannot handle arrays with more than one dimension");
                    }
                    frb.discard(4);
                    break;
                }
                ...
                default: {
                    if (t >= 0x7f && t <= 0xff) {
                        ...
                        throw new InternalClientError(`no codec implementation for EdgeDB data class ${t}`);
                    }
                }
            }
            return res;
        ...
        switch (t) {
            case CTYPE_BASE_SCALAR: {
                ...
                if (!res) {
                    if (KNOWN_TYPES.has(tid)) {
                        throw new InternalClientError(`no JS codec for ${KNOWN_TYPES.get(tid)}`);
                    }
                    throw new InternalClientError(`no JS codec for the type with ID ${tid}`);
                }
                if (!(res instanceof ScalarCodec)) {
                    throw new ProtocolError("could not build scalar codec: base scalar is a non-scalar codec");
                }
                break;
            }
            case CTYPE_SHAPE:
            case CTYPE_INPUT_SHAPE: {
                ...
                for (let i = 0; i < els; i++) {
                    ...
                    if (subCodec == null) {
                        throw new ProtocolError("could not build object codec: missing subcodec");
                    }
                    codecs[i] = subCodec;
                    names[i] = name;
                    flags[i] = flag;
            ...
            case CTYPE_SET: {
                ...
                const subCodec = cl[pos];
                if (subCodec == null) {
                    throw new ProtocolError("could not build set codec: missing subcodec");
                }
            ...
            case CTYPE_SCALAR: {
                ...
                res = cl[pos];
                if (res == null) {
                    throw new ProtocolError("could not build scalar codec: missing a codec for base scalar");
                }
                if (!(res instanceof ScalarCodec)) {
                    throw new ProtocolError("could not build scalar codec: base scalar has a non-scalar codec");
                }
                res = res.derive(tid);
                break;
            }
            case CTYPE_ARRAY: {
                ...
                const subCodec = cl[pos];
                if (subCodec == null) {
                    throw new ProtocolError("could not build array codec: missing subcodec");
                }
            ...
            case CTYPE_TUPLE: {
                ...
                if (els === 0) {
                    ...
                    for (let i = 0; i < els; i++) {
                        ...
                        const subCodec = cl[pos];
                        if (subCodec == null) {
                            throw new ProtocolError("could not build tuple codec: missing subcodec");
                        }
                        codecs[i] = subCodec;
                    }
            ...
            case CTYPE_NAMEDTUPLE: {
                ...
                for (let i = 0; i < els; i++) {
                    ...
                    const subCodec = cl[pos];
                    if (subCodec == null) {
                        throw new ProtocolError("could not build namedtuple codec: missing subcodec");
                    }
                    codecs[i] = subCodec;
                }
            ...
            case CTYPE_RANGE: {
                ...
                const subCodec = cl[pos];
                if (subCodec == null) {
                    throw new ProtocolError("could not build range codec: missing subcodec");
                }
        ...
        if (res == null) {
            if (KNOWN_TYPES.has(tid)) {
                throw new InternalClientError(`could not build a codec for ${KNOWN_TYPES.get(tid)} type`);
            }
            else {
                throw new InternalClientError(`could not build a codec for ${tid} type`);
            }
        }
