...
import { AccountEdit, AuthAction } from "lib/types/boid.system"
import { z } from "zod"
  ...
  .mutation(async(data) => {
    if (data.input.actions[0].name == "account.edit") {
      try {
        ...
        if (actData.social_ipfs_json != (await jsonToCID(json)).bytes.toString()) throw new Error("json ipfs cid mismatch")
