...
export const getInvites = route
  ...
  .query(async(req):Promise<(InviteCode & { _count:{ used:number } })[]> => {
    // TODO implement auth
...
export const getInvite = route
  ...
  .query(async data => {
    ...
    if (!exists || exists.sponsorBoidId != data.input.sponsorName) throw new Error("invalid code")
    else return exists
  })
...
export const useInvite = route
  ...
  .mutation(async req => {
    ...
    if (invite.disabled || invite.usesRemaining == 0 || invite.expires.getTime() < Date.now()) return new Error("invite code is inactive")
    const exists = await db.boidAccount.findUnique({ where: { boidId: params.boidId } })
    if (exists) return new Error("specified Boid ID already exists")
    const existingHash = await db.inviteCodeUsed.findUnique({ where: { emailHash: params.emailHash } })
    if (existingHash) return new Error("email already in use")

    // create the pending
    const pending = await db.newAccountPending.create({
      data: {
        createBoidId: params.boidId,
        emailHash: params.emailHash, // fix, need to generate hash internally
        key: params.key,
...
export const validatePendingInvite = route
  ...
  .mutation(async({ input }) => {
    ...
    if (!pendingCreate) return new Error("invalid code")
    if (pendingCreate.inviteCode.usesRemaining <= 0) return new Error("invite no longer valid, contact sponsor for new invite code")
    if (pendingCreate.executed) return new Error("account already created")

    // trigger the action to purchase the sponsored account on behalf of the sponsor
    const result = await sendAction(
      sysActions.buyAccount(
        {
          boid_id: pendingCreate.createBoidId,
          key: pendingCreate.key,
          sponsor: pendingCreate.inviteCode.sponsorBoidId
        }))
    if (!result) return new Error("error with create action")
    const finished = result.receipts.length > 0
    if (!finished) return new Error("create action failed:" + JSON.stringify(result.errors))
